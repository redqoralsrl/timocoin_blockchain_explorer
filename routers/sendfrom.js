...
router.post('/', function(req, res) {
    client.query("select * from users where id=?", [req.session.userId], (error, result) => {
        if(result[0].balance == 0) {
            res.send("<script language=\"javascript\">alert('[ ERROR ] :  Your balance is 0'); location.replace('/');</script>");
        }
        else {
